...
class Line:
    ...
    def __init__(self, buffer_len=10):
        # flag to mark if the line was detected in the last iteration
        self.detected = False
...
def get_fits_by_sliding_windows(birdeye_binary, line_lt, line_rt, n_windows=9):
    ...
    """
    height, width = birdeye_binary.shape  # (720, 1280)

    # Assuming you have created a warped binary image called "binary_warped"
    # Take a histogram of the bottom half of the image
    """this is not a histogram, which may be confusing 
    if we choose the name, histogram here"""
    histogram = np.sum(birdeye_binary[height // 2:-30, :], axis=0)  # (330, 1280) --> (1280,)
    ...
    midpoint = len(histogram) // 2  # 640
    leftx_base = np.argmax(histogram[:midpoint])  # 193
    rightx_base = np.argmax(histogram[midpoint:]) + midpoint  # 1137

    # Set height of windows, it should be a int type
    window_height = height // n_windows  # 720 // 9 = 80

    # Identify the x and y positions of all nonzero pixels in the image
    # nonzero(tuple) contains the indices of the no zero pixels in birdeye_binary,
    # nonzero[0] is the y coordinates of all nonzero pixels.
    nonzero_y, nonzero_x = birdeye_binary.nonzero()
...
def draw_back_onto_the_road(img_undistorted, Minv, line_lt, line_rt, keep_state):
    ...
    cv2.fillPoly(road_warp, np.int_([pts]), (0, 255, 0))  # fill the region with green color
